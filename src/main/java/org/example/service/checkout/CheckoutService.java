...
public class CheckoutService {
    ...
    public Either<ValidationErrors, RentalAgreement> checkout(
        ...
        return errors.hasErrors() ? Either.left(errors) : this.toolsDb.checkout(reservation.id(), reservation.tool().type()).mapLeft(e -> {
            LOGGER.error(
                    String.format(
            ...
            return new RentalAgreement(reservation.tool(), dates, reservation.price(), discount);
        });
    }
}
